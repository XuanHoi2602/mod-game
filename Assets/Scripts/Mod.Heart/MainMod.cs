...
namespace Mod.Heart
{
    public class MainMod : IActionListener, IChatable
    {
    	...
    	public static bool UpdateKey(int unused)
    	{
    		...
    		{
    			if (ModHeartPro.ExistItemBag(921))
    			{
                    UseItem(921);
                    return true;
                }
                if (ModHeartPro.ExistItemBag(454))
                {
                    UseItem(454);
                    return true;
                }
            ...
            {
                ModHeartPro.aGimBoss = !ModHeartPro.aGimBoss;
    			new Thread(ModHeartPro.AutoFocusBoss).Start();
    			GameScr.info1.addInfo("Auto gim boss: " + (ModHeartPro.aGimBoss ? "Bật" : "Tắt"), 0);
    			return true;
    		}
    		...
    		if (GameCanvas.keyAsciiPress == Hotkeys.SHIFT_U)
    		{
    			if (ModHeartPro.findBossMod)
    			{
                    ModHeartPro.findBossMod = false;
                }
    			if (ModHeartPro.tanCongBoss)
    			{
    				ModHeartPro.tanCongBoss = false;
    				ModHeartPro.listBossTrongKhu.Clear();
    			}
    			if (ModHeartPro.tieuDietNguoiBatCo)
    			{
    				ModHeartPro.tieuDietNguoiBatCo = false;
    				ModHeartPro.listNguoiCoDen.Clear();
    			}
    			if (ModHeartPro.DoSatBossNapa) {
    				ModHeartPro.DoSatBossNapa = false;
    			}
    			if (ModHeartPro.aWhis)
    			{
    				ModHeartPro.aWhis = false;
    			}
    			if (AutoboMong.autoboMong)
    			{
