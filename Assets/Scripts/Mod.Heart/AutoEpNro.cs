...
namespace Mod.Heart
{
    public class AutoEpNro : IActionListener
    {
    	...
    	{
    		Item item = ModHeartPro.FindItemBag(id);
    		if (item != null)
    		{
    	...
    	public static void epNro(int idItem)
    	{
    		...
    		if (soLuongItem(idItem - 1) == 0 && ModHeartPro.countEmptyBag() == 0)
    		{
    		...
    		while (checkItempara7(idItem) && num < num2)
    		{
    			...
    			ModHeartPro.teleNPC(21);
    			Service.gI().openMenu(21);
    			Service.gI().confirmMenu(21, 6);
    			GameCanvas.panel.vItemCombine.addElement(ModHeartPro.FindItemBag(idItem));
    			Thread.Sleep(100);
    	...
    	{
    		while (ModHeartPro.FindItemBag(idItem) != null)
    		{
    			epNro(idItem);
    			Thread.Sleep(1000);
    		}
