...
namespace Mod.Heart
{
    public class ShowSetKH
    {
        ...
        {
            while (ModHeartPro.catDoVIP)
            {
                int num = ModHeartPro.countEmptyBox();
                if (num == 0)
                {
                    GameScr.info1.addInfo("|2| Rương FULL rồi", 0);
                    ModHeartPro.catDoVIP = false;
                    break;
                }
                if (ModHeartPro.isFULLBag() && !ModHeartPro.isFULLBox())
                {
                    ModHeartPro.tieuDietNguoiBatCo = false;
                    ModHeartPro.tanCongBoss = false;
                    bool flag = false;
                    ...
                    for (int num2 = Char.myCharz().arrItemBag.Length - 1; num2 >= 0; num2--)
                    {
                        ...
                        Thread.Sleep(400);
                        if (ModHeartPro.isFULLBox())
                        {
                            ModHeartPro.catDoVIP = false;
                            break;
                        }
                    }
                    if (num == ModHeartPro.countEmptyBox())
                    {
