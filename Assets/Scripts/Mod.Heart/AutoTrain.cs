...
namespace Mod.Heart
{
    public class AutoTrain : IActionListener, IChatable
    {
        ...
        public static void Update()
        {
            if (trainCuongLe)
            {
                ...
                if (checkPointEnd)
                {
                    ...
                    {
                        Char.myCharz().mobFocus = GetNextMobHeart();
        ...
        public static void TuMoTDLT()
        {
            try
            {
                if (!ModHeartPro.ExistItemBag(521))
                {
                    return;
                }
        ...
        {
            bool flag = (!AutoTrain.isAutoTrain && !GameScr.canAutoPlay) || Char.myCharz().statusMe == 14 || Char.myCharz().statusMe == 5 || ModHeartPro.tanCongBoss || ModHeartPro.tieuDietNguoiBatCo || AutoMap.isXmaping;
            if (!flag)
            {
        ...
        private static Mob GetNextMobHeart()
        {
